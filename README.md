...
点击即可展开按 token 桶拆分的明细。价格直接取自 DeepSeek 官方价目表，**以人民币计价**，
不做任何汇率换算。

![会话头部的花费标记与展开的明细面板](docs/screenshot.jpg)

图中为高峰时段：会话头部右侧是默认收起的花费标记，点开后是按 token 桶拆分的明细。
...
官方价目表只有「缓存命中 / 缓存未命中」两档，而首次把前缀写入缓存的那次请求本身就是未命中，
所以 `cacheWriteTokens` 按未命中价计费；又因为四个桶互斥，不会与 `uncachedInputTokens` 重复计费。

### 已知口径限制

- **跨时段的历史会按当前时段重算。** 投影只给出累计 token 数，不记录「每个 token 产生于什么
...
挂载点是 `conversation.session.header.actions` 槽位（`order: 15`），属于会话作用域。

数据来源不依赖该槽位恰好提供哪些标准 props：插件通过**槽位自己的 `inject`**，从
...
会话绑定缺失时的降级、四桶计费与缓存写入口径、金额格式化、高峰判定的时区比对、
flash 空闲 / flash 高峰 / pro 空闲三档的端到端渲染金额，以及点击展开明细面板。此外还钉住两处
容易静默失效的地方：
