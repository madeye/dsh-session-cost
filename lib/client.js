window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 把模型 id 归一到价目档位。历史模型名（deepseek-v4-flash、
		 * deepseek-chat 等）都按 flash 档计价，因为官方已把它们路由到 Flash。
		...
		/**
		 * 按四个互不重叠的 token 桶计算费用。
		 *
		 * `dsh-token-meter` 的 TokenUsageProjection 保证四桶互斥；官方价目表只有
		 * 「缓存命中 / 缓存未命中」两档，因此**缓存写入按未命中价计费**——首次把
		 * 前缀写进缓存的那次请求本来就是未命中。
		 *
		 ...
		 * 会话头部的实时花费条目。
		 *
		 * token 用量来自 `tokenUsage` 会话投影（宿主侧回放持久日志得出，客户端只做
		 * 展示），模型来自 `modelSelection` 投影。两者都通过本插件自己的 slot
